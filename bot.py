...
#             openai python-dotenv edge-tts fastapi uvicorn
...
# ── Load environment variables FIRST before importing core AI modules ──
load_dotenv()
...
ai_client = OpenAI(
    ...
    api_key=GROQ_KEY or "dummy-key-for-init",
)
...
vc_response_cooldown: dict = {}
VC_COOLDOWN_SECONDS = 8
...
═══ KNOWLEDGE & ACCURACY ═══
You have broad, reliable knowledge in science, math, coding, history, pop culture, sports, gaming, and advice.

RULES:
- Answer confidently when you know. Don't hedge with "as an AI I can't..." — just answer.
- If genuinely unsure, say so in the person's style.
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# AI CALLS — ASYNC WRAPPERS FOR UNBLOCKED LOOP
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
async def async_call_ai(system: str, history: list, user_msg: str, max_tokens: int = 800) -> str:
    return await asyncio.to_thread(call_ai, system, history, user_msg, max_tokens)

def _quick_ai_sync(prompt: str, max_tokens: int = 500) -> str:
...
async def quick_ai(prompt: str, max_tokens: int = 500) -> str:
    return await asyncio.to_thread(_quick_ai_sync, prompt, max_tokens)
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# TTS — edge-tts → ffmpeg → VC (MUTEX LOCKED)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
async def speak_in_vc(vc: discord.VoiceClient, text: str, mood: str = "chill/neutral"):
    ...
    session  = voice_sessions.get(guild_id) if guild_id else None
    lock     = session.get("speaking_lock") if session else None
...
async def handle_query(user_id, guild_id, username, channel_id, prompt,
    ...
    response = await async_call_ai(system, history, prompt, max_tokens=max_tok)
    ...
    if not is_voice:
        for chunk in [response[i:i+1990] for i in range(0, len(response), 1990)]:
            await safe_send(reply_fn, chunk) if hasattr(reply_fn, 'send') else await reply_fn(chunk)
    ...
    class FGOSSink(BasicSink):
        ...
        def _on_audio(self, user, data):
            try:
                ...
                # Thread-safe scheduling from voice thread
                loop = self.bot.loop
                task = loop.call_soon_threadsafe(
                    loop.call_later, 1.2, lambda user=user: asyncio.run_coroutine_threadsafe(
        ...
        async def _process(self, user, pcm_bytes: bytes):
            try:
                ...
                try:
                    ...
                    print(f"[VC] WAV conversion error: {wav_err}")
                    return
    ...
    for guild_id, session in list(voice_sessions.items()):
        try:
            ...
            if hasattr(vc, 'router') and vc.router:
                if hasattr(vc.router, 'is_alive') and not vc.router.is_alive():
                    print(f"[VC] Router died for guild {guild_id}")
        except Exception as e:
...
async def conversation_starter():
    for channel_id in list(starter_channels):
        try:
            ...
            msg = await quick_ai(prompt, max_tokens=60)
...
async def on_ready():
    ...
    print(f"Voice recv: {'ENABLED' if VOICE_RECV_AVAILABLE else 'DISABLED'}")
    print("=" * 50)
...
async def on_voice_state_update(member, before, after):
    ...
    if not getattr(before, "self_stream", False) and getattr(after, "self_stream", False):
        ...
        try:
            reaction = await quick_ai(prompt, max_tokens=60)
...
async def sync_cmd(ctx):
    await safe_defer(ctx)
    await bot.tree.sync()
    await safe_send(ctx, "slash commands synced!")
...
async def ask_fg_os(ctx, *, question: str):
    ...
    try:
        ...
        await safe_send(ctx, f"⚠️ Error: {e}")
...
async def clear_ctx(ctx):
    ...
    conn.commit()
    conn.close()
    await safe_send(ctx, "memory cleared")
...
async def setnick(ctx, *, nickname: str):
    ...
    await safe_send(ctx, f"got it, calling you {nickname.strip()} from now on")
...
async def my_memory(ctx):
    ...
    if not memories:
        await safe_send(ctx, "don't remember anything specific yet, talk more", ephemeral=True)
        return
...
async def clear_memory(ctx):
    ...
    conn.commit()
    conn.close()
    await safe_send(ctx, "cleared everything i knew about u")
...
async def stats(ctx, member: discord.Member = None):
    ...
    conn.close()
    if total == 0:
        await safe_send(ctx, f"no messages logged for {target.display_name} yet")
        return
    words = []
    for m in all_msgs:
...
async def leaderboard(ctx):
    ...
    conn.close()
    if not rows:
        await safe_send(ctx, "no data yet, people gotta talk first")
        return
...
async def servervibe(ctx):
    ...
    if len(server_msgs) < 5:
        await safe_send(ctx, "not enough messages logged yet")
        return
    sample = "\n".join(
        f"{u}: {m[:200]}"
    ...
    try:
        vibe  = await quick_ai(prompt, max_tokens=180)
...
async def tldr(ctx, *, text: str):
    ...
    try:
        ...
        result = await quick_ai(prompt, max_tokens=120)
        await safe_send(ctx, f"**TLDR:** {result}")
    except Exception as e:
        await safe_send(ctx, f"⚠️ Error: {e}")
...
async def remind(ctx, *, text: str):
    ...
    try:
        ...
        result = await quick_ai(prompt, max_tokens=60)
        match  = re.search(r'\{.*?\}', result, re.DOTALL)
        if not match:
            await safe_send(ctx, "couldn't parse — try 'remind me to X in 2 hours'", ephemeral=True)
            return
        ...
        await safe_send(ctx, f"got it, reminding you about **{task}** in {time_str}", ephemeral=True)
    except Exception as e:
        await safe_send(ctx, f"⚠️ Error: {e}", ephemeral=True)
...
async def roast(ctx, member: discord.Member):
    ...
    if len(target_msgs) < 5:
        await safe_send(ctx, f"not enough msgs from {member.display_name} to roast them, they gotta talk more")
        return
    sample = "\n".join(target_msgs[-20:])
    prompt = (
    ...
    try:
        roast_text = await quick_ai(prompt, max_tokens=130)
        await safe_send(ctx, f"🔥 {member.mention} {roast_text}")
    except Exception as e:
        await safe_send(ctx, f"⚠️ Error: {e}")
...
async def impersonate(ctx, member: discord.Member, *, situation: str):
    ...
    if len(target_msgs) < 5:
        await safe_send(ctx, f"not enough messages from {member.display_name}, they gotta talk more first")
        return
    ...
    try:
        response = await quick_ai(prompt, max_tokens=180)
        await safe_send(ctx, f"**{member.display_name}:** {response}")
    except Exception as e:
        await safe_send(ctx, f"⚠️ Error: {e}")
...
async def debate(ctx, *, topic: str):
    ...
    try:
        ...
        result = await quick_ai(prompt, max_tokens=250)
...
async def starterchannel(ctx):
    ...
    if cid in starter_channels:
        ...
        await safe_send(ctx, "convo starters OFF")
    else:
        starter_channels.add(cid)
        await safe_send(ctx, "convo starters ON — dropping one every ~45 min")
...
async def battleship(ctx, opponent: discord.Member = None):
    ...
    if is_ai:
        ...
        if opponent.id == ctx.author.id:
            await safe_send(ctx, "you can't play against yourself, goofball")
            return
        ...
        board_str = game.get_board_string(for_player2=False)
        try:
...
async def fire(ctx, column: str, row: str):
    ...
    if not game:
        await safe_send(ctx, "you're not in an active battleship game. start one with `/battleship`")
        return
    
    if game.game_state == "finished":
        await safe_send(ctx, "game already finished. start a new one with `/battleship`")
        return
    ...
    try:
        ...
        if not (0 <= col_idx < 10 and 0 <= row_idx < 10):
            await safe_send(ctx, "❌ Coordinates out of bounds! Use A-J for columns, 1-10 for rows")
            return
    except:
        await safe_send(ctx, "❌ Invalid format! Use: `/fire A 5`")
        return
    ...
    if not (is_player1 or is_player2):
        await safe_send(ctx, "❌ You're not in this game!")
        return
    
    if game.is_ai:
        if not is_player1:
            await safe_send(ctx, "❌ This is a single-player game!")
            return
        if game.current_turn != 1:
            await safe_send(ctx, "🤔 AI is calculating next move...")
            return
    else:
        ...
        if (is_player1 and game.current_turn != 1) or (is_player2 and game.current_turn != 2):
            await safe_send(ctx, f"⏳ It's {current_player_name}'s turn!")
            return
    
    result, ship, is_sunk, all_sunk = game.shoot(is_player1, row_idx, col_idx)
    
    if result == "already_shot":
        await safe_send(ctx, f"💧 Already shot {column.upper()}{row}! Try another spot.")
        return
    ...
    if game.is_ai and game.game_state != "finished":
        ...
        try:
            ...
            await safe_send(ctx, "⚠️ AI had a moment... trying again")
            return
        ...
        try:
            ...
            await safe_send(ctx, "⚠️ Something went wrong with the AI turn")
            return
...
async def gameboard(ctx):
    ...
    game = get_game(ctx.author.id)
    if not game:
        await safe_send(ctx, "you're not in an active battleship game!", ephemeral=True)
        return
    ...
    await safe_send(ctx, embed=status_embed, ephemeral=True)
    await safe_send(ctx, embed=ship_embed, ephemeral=True)
    await safe_send(ctx, f"```\n{board_str}\n```", ephemeral=True)
...
async def quitgame(ctx):
    ...
    game = get_game(ctx.author.id)
    if not game:
        await safe_send(ctx, "you're not in a battleship game")
        return
...
async def joinvc(ctx):
    if not ctx.author.voice or not ctx.author.voice.channel:
        await safe_send(ctx, "you're not in a VC")
        return
    await safe_defer(ctx)
    ...
    try:
        ...
        await safe_send(ctx, f"couldn't connect: {e}")
        return
    ...
    if VOICE_RECV_AVAILABLE and isinstance(vc, VoiceRecvClient):
        try:
            ...
            try:
                ...
                msg += " — joined but voice listening failed"
                sink = None
        ...
        msg += " — joined (voice receive disabled/unavailable)"
...
async def say(ctx, *, text: str):
    ...
    if not session or not session["vc"].is_connected():
        await safe_send(ctx, "i'm not in a VC — use /joinvc first", ephemeral=True)
        return
    mood = detect_mood(text)
    await speak_in_vc(session["vc"], text, mood)
    await safe_send(ctx, f"🔊 said: *{text[:100]}*", ephemeral=True)
...
async def trashtalk(ctx, member: discord.Member):
    ...
    if not all_vc and len(msgs) < 3:
        await safe_send(ctx, f"no receipts on {member.display_name} yet — need them to talk in VC or send more messages")
        return
    ...
    try:
        trash = await quick_ai(prompt, max_tokens=150)
        await safe_send(ctx, f"🗑️ {member.mention} {trash}")
...
async def vctranscript(ctx, member: discord.Member = None):
    ...
    if not all_t:
        await safe_send(ctx, f"no VC transcript for {target.display_name} yet", ephemeral=True)
        return
